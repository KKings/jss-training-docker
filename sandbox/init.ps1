...
Param (
    ...
    $LicenseXmlPath = ".\license",
    
    [Parameter(Mandatory = $false)]
    [string]
    [ValidateNotNullOrEmpty()]
    $cmDomain = "admin.sc10.local",
        
    [Parameter(Mandatory = $false)]
    [string]
    [ValidateNotNullOrEmpty()]
    $idDomain = "login.sc10.local",
    
    [Parameter(Mandatory = $false)]
    [string]
    [ValidateNotNullOrEmpty()]
    $siteDomain = "trainer.sc10.local",
...
Push-Location docker\build\traefik\certs
try {
    $mkcert = ".\mkcert.exe"
    ...
    & $mkcert -install
    & $mkcert -cert-file "$($cmDomain).crt" -key-file "$($cmDomain).key" $cmDomain
    & $mkcert -cert-file "$($idDomain).crt" -key-file "$($idDomain).key" $idDomain
    & $mkcert -cert-file "$($siteDomain).crt" -key-file "$($siteDomain).key" $siteDomain
}
catch {
...
Add-HostsEntry $cmDomain
Add-HostsEntry $idDomain
Add-HostsEntry $siteDomain
